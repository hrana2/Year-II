...
(* This function searches for the class c_name in the class enviroemnt 
If the class does not exsist then it return an error. 
Otherwise it returns (super, fields, methods). 
This is done by using  mem_assoc which return true or false 
whether a binding exsits for a given key. If its true then return 
all of the triples else error statemnt.*)
...
(* This function creates a new enviroment for the created 
object called fields. It exteends the enviroment and also 
creates a dummy value of NunVal 0 for every entry in the store. 
This function returns an ea_result that ignores the enviroment and 
just builds a new one.  *)
(* folds over the contents of the provided list and extedns each one *)
...
(* This functions looks for and applies the method 
This is done by looking for the method in the class 
enviroment in the class c_name. If c_name is found 
then you dont have to look at the super class. This is 
done by using mem_assoc then retruning Some() otherwise 
None *)
...
and
  ...
  (* First, evaluate es which will make a list of expressed vals
    which will be passed to the method. Next, lookup the class in the 
    enviroment with the help of lookup_class. Create the enviroment 
    with new_env. If there is an 'initialize' method then apply with 
    given method. *)
  | NewObject(c_name,es) ->
  ...
  (* Evaluate e and make sure its an object 
    Evaluate es which will make a list of args 
    and passed onto the method. look and apply method 
    by using match  *)
  | Send(e,m_name,es) ->
     eval_expr e >>= fun s ->
    ...
    match lookup_method c_name m_name !g_class_env with
      | None -> error "cannot find method"
      | Some m -> apply_method m_name s args m
    end
  | Self ->
    eval_expr (Var "_self")
  (* Evaluate es which makes a list of args which will 
  be passed onto the method. Lookup the super class and 
  makes sure its a string. Find who self is in curr enviroment
  look and apply emthod with given func  *)
  | Super(m_name,es) ->
    sequence (List.map eval_expr es) >>= fun args -> 
